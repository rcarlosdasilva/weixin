...
class Weixin private constructor() {
  ...
  class Registry private constructor() {
    ...
    companion object {
      const val OP_ACCOUNT_CACHE_KEY = "unique"
      private val instance = Registry()

      /**
       * 全局配置
       */
      @JvmStatic
      fun with(setting: Setting) {
      ...
      @JvmStatic
      fun checkin(op: Op) {
        CacheHandler.of(Op::class.java).put(OP_ACCOUNT_CACHE_KEY, op)
        instance.hasOp = true
      }
      ...
      fun checkin(mp: Mp) {
        ...
        val verified = when {
          !mp.proxyWithOp && mp.appSecret?.isBlank() == true -> {
            instance.logger.warn { "未找到公众号的app_scret，该公众号将不被注册" }
            false
          }
          !mp.proxyWithOp && !instance.hasOp -> {
            instance.logger.warn { "未找到开放平台信息，但该公众号配置为使用开放平台，将不被注册" }
            false
          }
          mp.proxyWithOp && mp.refreshToken?.isBlank() == true -> {
            instance.logger.warn { "未找到开放平台授权方的刷新令牌authorizer_refresh_token，该公众号将不被注册" }
            false
          }
          mp.mpId?.isBlank() == true -> {
            instance.logger.warn { "公众号[appid=${mp.appId}]未设置Mp.mpId，当微信通知回调或其他操作的时候，不设置有可能会导致无法正确找到对应的公众号信息" }
            true
          }
          mp.appId == mp.key -> {
            instance.logger.warn { "未设置Mp.key，将使用公众号的appid：[${mp.appId}]作为默认key" }
            true
          }
          else -> true
        }
        ...
        if (verified) {
          ...
          instance.logger.info { "注册公众号：[KEY: ${mp.key}, APPID: ${mp.appId}]" }
        }
      }
      ...
      fun checkin(listener: WeixinListener) {
        instance.listeners[listener.javaClass.name] = listener
      }
      ...
      fun remove(key: String) {
        lookup(key)?.let {
          ...
          instance.logger.info("注销公众号信息：[$key]")
        }
      }
